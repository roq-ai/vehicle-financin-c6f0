...
import { getLoanOfficerById, updateLoanOfficerById } from 'apiSdk/loan-officers';
...
function LoanOfficerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<LoanOfficerInterface>(
    () => (id ? `/loan-officers/${id}` : null),
    () => getLoanOfficerById(id),
  );
  ...
  const handleSubmit = async (values: LoanOfficerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateLoanOfficerById(id, values);
      mutate(updated);
      resetForm();
