...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!session) {
    if (req.method === 'GET') {
      return getCustomerServiceRepresentativesPublic();
    }
  ...
  async function getCustomerServiceRepresentativesPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'customer_service_representative');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.customer_service_representative.count(countOptions as unknown),
      prisma.customer_service_representative.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
