...
export function LoanListPage(props: LoanListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getLoans({
        relations: ['user', 'lender', 'vehicle.count'],
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'monthly_emi', header: 'monthly_emi', accessorKey: 'monthly_emi' },
    {
      id: 'disbursal_date',
      header: 'Disbursal Date',
      accessorKey: 'disbursal_date',
      cell: ({ row: { original: record } }: any) =>
        record?.disbursal_date ? format(parseISO(record?.disbursal_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('lender', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'lender',
          header: 'Lender',
          accessorKey: 'lender',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/lenders/view/${record.lender?.id}`}>
              {record.lender?.name}
            </Link>
          ),
        }
